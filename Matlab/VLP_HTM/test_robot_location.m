...
load('Clustering\epsilon_minPts_matrix_62_exp_4')
...
xloc =1.28 %rand()*(params.W)
yloc =1.28 %rand()*params.L
...
epsilon = 0.002%interp2(x_data,y_data,epsilon_matrix,estimatedLocation(1),estimatedLocation(2),'linear');
...
% Plot Results
figure
legends = PlotClusterinResult(coordinates, IDX);
title(['DBSCAN Clustering (\epsilon = ' num2str(epsilon) ', MinPts = ' num2str(minPts) ') at x= ' num2str(xloc) 'm, y= ' num2str(yloc) 'm']);

hold on
%plot real location for comparison
plot(estimatedLocation(1), estimatedLocation(2),'*b')
plot(xloc, yloc,'*m');
xlabel('X(m)')
ylabel('Y(m)')

legend([ legends 'Trilat. Location' 'Real Location'])
axis([0 4 0 4])
...
% % % % 
% % % % 
% % % % epsilon = interp2(x_data,y_data,epsilon_matrix,smallest_error_cluster_pos(1),smallest_error_cluster_pos(2),'linear')
% % % % minPts = round(interp2(x_data,y_data,minPts_matrix,smallest_error_cluster_pos(1),smallest_error_cluster_pos(2),'linear'))
% % % % 
% % % % IDX=DBSCAN(coordinates,epsilon,minPts);
% % % % 
% % % % %% Plot Results
...
% % % % 
% % % % % display(['medio clusters: ' num2str(mean(coordinates(logical(IDX),:)))]); %average position of all the clusters
% % % % display(['medio menor error a estimativa:' num2str(mean(coordinates(logical(IDX==smallest_error_index),:)))])
% % % % % display(['medio maior cluster:' num2str(mean(coordinates(logical(IDX==largest_index),:)))])
% % % % 
% % % % display(['Estimativa loca trilat:' num2str(estimatedLocation)]);
% % % % display(['Real loc:' num2str([xloc yloc])])
